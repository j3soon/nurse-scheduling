...
Deployments configured with AI chat history additionally store each turn's full conversation record in PostgreSQL: the user's messages with attachment filenames, queued follow-up messages, the assistant's text and reasoning, the tools it called with their arguments and results, and proposal approval or rejection decisions, in order. Tool results can contain schedule content, up to the whole schedule, and text read from attachments. Each turn also records the model, timestamps, attachment count, token usage, and status, keyed by a chat session and the administrative credential ID when authentication is enabled. Raw attachment files and images are not stored in chat history, and schedules are not stored separately from those tool results. Stored turns are deleted after the operator's configured retention window, 30 days by default. Operators configure their own backups and backup retention separately.
