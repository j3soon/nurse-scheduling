...
request after its five-second pause-request deadline and does not retry it. The
longer background deadline does not delay foreground work because new activity
cancels an in-progress pause. A timeout has an unknown remote outcome, so the
next operation performs the separately bounded auto-resume probe before doing
work. A later idle pause may still be attempted because the control-plane
...
pytest -q tests/test_ai_sandbox.py tests/test_ai_sandbox_agent.py \
  tests/test_ai_pi_bash.py tests/test_ai_pi_edit.py tests/test_ai_pi_read.py \
  tests/test_ai_pi_write.py tests/test_ai_sandbox_tools.py \
