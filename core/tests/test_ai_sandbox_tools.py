...
def test_sandbox_edit_resolves_relative_paths_and_replaces_exact_text():
    backend = FakeSandboxBackend("fake-1", initial_files={"/workspace/notes.txt": b"one\ntwo\n"})
    tools = SandboxPiTools(backend, 10)
    ...
    outcome = asyncio.run(
        tools.execute(
            EDIT_TOOL,
            '{"path":"notes.txt","edits":[{"oldText":"two","newText":"TWO"}]}',
        )
    )

    assert outcome.text == "Successfully replaced 1 block(s) in notes.txt."
    assert outcome.ok
    assert backend.files["/workspace/notes.txt"] == b"one\nTWO\n"


def test_sandbox_edit_reports_model_correctable_matching_errors():
    backend = FakeSandboxBackend("fake-1", initial_files={"/workspace/notes.txt": b"same same"})
    tools = SandboxPiTools(backend, 10)

    duplicate = asyncio.run(
        tools.execute(
            EDIT_TOOL,
            '{"path":"notes.txt","edits":[{"oldText":"same","newText":"new"}]}',
        )
    )
    missing = asyncio.run(tools.execute(EDIT_TOOL, '{"path":"missing.txt","edits":[{"oldText":"x","newText":"y"}]}'))

    assert not duplicate.ok
    assert "Found 2 occurrences" in duplicate.text
    assert not missing.ok
    assert "Sandbox file not found" in missing.text
    assert backend.files["/workspace/notes.txt"] == b"same same"
