...
test('quick-add drag selection applies the same shift request across multiple cells', async ({ page }) => {
  /*
   * Steps:
   * 1. Confirm the request summary is empty before quick-add mode starts.
   * 2. Enable quick-add, choose one shift type and weight, then drag across two request cells.
   * 3. Release the mouse to commit the drag gesture.
   * 4. Confirm the grouped request remains after the gesture completes.
   */
  await disableModalDialogs(page);
  await seedSchedulingState(page, {
    apiVersion: 'test',
    description: 'drag multiselect seed',
    dates: {
