...
test('undo and redo keyboard shortcuts restore prior scheduling state across page actions', async ({ page }) => {
  /*
   * Steps:
   * 1. Reset to the default schedule, add a person through the real page form, and confirm it appears.
   * 2. Trigger the global undo shortcut and confirm the person disappears.
   * 3. Trigger the global redo shortcut and confirm the person returns.
   */
  ...
  await expect(page.getByText('Night Owl')).toHaveCount(0);
