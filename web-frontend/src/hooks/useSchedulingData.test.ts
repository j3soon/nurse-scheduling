...
function createPopulatedHistoryState() {
  const state = {
    apiVersion: 'alpha',
    description: '',
    dates: {
      ...
      items: [],
      groups: [
        { id: 'WORKDAY', members: [], description: 'Workdays' },
        { id: 'FREEDAY', members: [], description: 'Freedays' },
      ],
    },
    people: {
  ...
  return { state, history: [state], currentHistoryIndex: 0 };
}
...
describe('useSchedulingData', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
    localStorage.clear();
    localStorage.setItem(STORAGE_KEY, JSON.stringify(createPopulatedHistoryState()));
  });
  ...
  it('falls back to the default state and logs when localStorage.getItem throws', async () => {
    ...
    await waitFor(() => {
      ...
      expect(result.current.shiftTypeData.items.filter(item => !item.isAutoGenerated)).toEqual([]);
      expect(result.current.dateData.groups.filter(group => !group.isAutoGenerated)).toEqual([]);
      expect(result.current.preferences).toEqual([{ type: AT_MOST_ONE_SHIFT_PER_DAY }]);
    });
  ...
  it('resets to defaults via createNewState after mutations', async () => {
    ...
    await waitFor(() => {
      expect(result.current.peopleData.items).toEqual([]);
      expect(result.current.peopleData.groups.filter(group => !group.isAutoGenerated)).toEqual([]);
      expect(result.current.shiftTypeData.items.filter(item => !item.isAutoGenerated)).toEqual([]);
      expect(result.current.shiftTypeData.groups.filter(group => !group.isAutoGenerated)).toEqual([]);
      expect(result.current.dateData.groups.filter(group => !group.isAutoGenerated)).toEqual([]);
      expect(result.current.preferences).toEqual([{ type: AT_MOST_ONE_SHIFT_PER_DAY }]);
    });
  });
