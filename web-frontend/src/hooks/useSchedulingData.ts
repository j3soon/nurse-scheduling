...
function _generateDateItems(startDate: string, endDate: string): Item[] {
  const dates: Item[] = [];
...
function addAutoGeneratedItemsAndGroupsToState(state: SchedulingState): SchedulingState {
  const filteredState = filterAutoGeneratedState(state);
  return {
    ...state,
    dates: {
      ...state.dates,
      items: [...filteredState.dates.items, ..._generateAutoGeneratedItems(DataType.DATES, state.dateRange)],
      groups: [...filteredState.dates.groups, ..._generateAutoGeneratedGroups(DataType.DATES, state.dates.items, state.dateRange)]
    },
    people: {
      ...state.people,
...
function createDefaultHistoryState(): HistoryState {
  const defaultState = addAutoGeneratedItemsAndGroupsToState(createDefaultState());
  return {
    state: defaultState,
...
function loadStateFromStorage(): HistoryState {
  ...
  try {
    ...
    const currentState = addAutoGeneratedItemsAndGroupsToState(parsedHistoryState.state);
...
export function useSchedulingData() {
  ...
  const updateState = (updater: (prevState: SchedulingState) => SchedulingState) => {
    setHistoryState(prevHistoryState => {
      const newState = addAutoGeneratedItemsAndGroupsToState(updater(prevHistoryState.state));
  ...
  const moveHistoryIndex = (delta: number) => {
    setHistoryState(prevHistoryState => {
      ...
      const restoredState = addAutoGeneratedItemsAndGroupsToState(targetState);
  ...
  const createNewState = () => {
    const newState = addAutoGeneratedItemsAndGroupsToState(createDefaultState());
    const newHistoryState = {
      state: newState,
      history: [newState],
      currentHistoryIndex: 0
  ...
  const loadFromYaml = (yamlData: SchedulingState) => {
    // Create new state from YAML data without validation
    // TODO: Add validation
    const newState: SchedulingState = addAutoGeneratedItemsAndGroupsToState({
      dateRange: yamlData.dateRange || { startDate: undefined, endDate: undefined },
      dates: yamlData.dates || { items: [], groups: [] },
      people: yamlData.people || { items: [], groups: [] },
      shiftTypes: yamlData.shiftTypes || { items: [], groups: [] },
      shiftTypeRequirements: yamlData.shiftTypeRequirements || [],
      shiftRequestPreferences: yamlData.shiftRequestPreferences || [],
      shiftTypeSuccessions: yamlData.shiftTypeSuccessions || [],
      shiftCounts: yamlData.shiftCounts || []
    });
