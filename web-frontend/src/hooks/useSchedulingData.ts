...
function replaceLatestHistoryEntry(currentHistoryState: HistoryState, newState: SchedulingState): HistoryState {
  const trimmedHistory = currentHistoryState.history.slice(0, currentHistoryState.currentHistoryIndex + 1);
...
export function useSchedulingData() {
  ...
  const updateState = (
    ...
    setHistoryState(prevHistoryState => {
      ...
      const newHistoryState = options?.replaceLatestHistoryEntry
        ? replaceLatestHistoryEntry(prevHistoryState, newState)
        : addToHistory(prevHistoryState, newState);
  ...
  const loadFromYaml = (yamlData: unknown) => {
    ...
    const newState: SchedulingState = addAutoGeneratedToState({
      ...
      people: data.people || { items: [], groups: [], history: [] },
