...
import { AUTO_GENERATED_GROUPS, isReservedKeyword, filterAutoGenerated } from '@/utils/keywords';
...
function generateAutoGeneratedGroups(dataType: DataType, items: Item[]): Group[] {
  return AUTO_GENERATED_GROUPS[dataType].map(autoGroup => ({
    id: autoGroup.id,
    members: autoGroup.generateMembers(items),
...
function addAutoGeneratedGroupsToState(state: SchedulingState): SchedulingState {
  ...
  return {
    ...
    dates: {
      ...state.dates,
      groups: [...filteredState.dates.groups, ...generateAutoGeneratedGroups(DataType.DATES, state.dates.items)]
    },
    people: {
      ...state.people,
