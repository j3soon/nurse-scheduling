...
function createDefaultState(): SchedulingState {
  ...
  return {
    ...
    export: generateExportLayoutConfig(shiftTypes, dates.groups)
  };
}
...
function loadStateFromStorage(): HistoryState {
  ...
  try {
    ...
    for (const state of [...parsedHistoryState.history, parsedHistoryState.state]) {
      ...
      state.export = state.export || { formatting: [] };
      state.export.formatting = state.export.formatting || [];
      state.export.extraColumns = state.export.extraColumns || [];
      state.export.extraRows = state.export.extraRows || [];
    }
