...
describe('SaveAndLoadPage', () => {
  ...
  const readAsTextCalls: Array<{ fileName: string; encoding?: string }> = [];

  class MockFileReader {
    onload: ((ev: ProgressEvent<FileReader>) => unknown) | null = null;

    readAsText(file: File, encoding?: string) {
      readAsTextCalls.push({ fileName: file.name, encoding });
  ...
  it('loads uploaded YAML with a real UTF-8 BOM without a corruption prompt', async () => {
    ...
    const input = document.querySelector('input[type="file"]') as HTMLInputElement;
    const file = new File(['ignored'], 'bom.yaml', { type: 'application/x-yaml' });
    ...
    await waitFor(() => {
      expect(loadFromYaml).toHaveBeenCalledWith(
        expect.objectContaining({ apiVersion: 'alpha', description: 'utf8-bom' }),
      );
    });
    expect(confirm).not.toHaveBeenCalled();
  });

  it('asks before recovering BOM-corrupted YAML prefixes in uploaded files', async () => {
    const yamlSpy = vi.spyOn(yaml, 'load');

    renderSaveAndLoadPage();

    const input = document.querySelector('input[type="file"]') as HTMLInputElement;
    const file = new File(['ignored'], 'mojibake.yaml', { type: 'application/x-yaml' });
    ...
    expect(confirm).toHaveBeenCalledWith(expect.stringContaining('Corrupted YAML header detected.'));
    expect(confirm).toHaveBeenCalledWith(expect.stringContaining('wrong text encoding'));
    expect(yamlSpy).toHaveBeenCalledWith(expect.stringMatching(/^apiVersion: alpha/));
  });

  it('does not parse uploaded YAML when corrupted header recovery is cancelled', () => {
    const yamlSpy = vi.spyOn(yaml, 'load');
  ...
  it('asks about corrupted headers before app version mismatches', async () => {
    ...
    const input = document.querySelector('input[type="file"]') as HTMLInputElement;
    const file = new File(['ignored'], 'mojibake-old-version.yaml', { type: 'application/x-yaml' });
    fileContentsByName.set('mojibake-old-version.yaml', '嚜瘸piVersion: alpha\ndescription: mojibake\nappVersion: v0.0.1\n');
  ...
  it('downloads an item-only anonymized snapshot without changing the current YAML', async () => {
    ...
    expect(mockGenerateYamlFromState).toHaveBeenCalledWith(expect.objectContaining({
      people: {
