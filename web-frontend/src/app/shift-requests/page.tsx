...
export default function ShiftRequestsPage() {
  ...
  const getHistoryValue = (history: string[] | undefined, columnIndex: number): string => {
    const h = history ?? [];
    const offset = historyColumnsCount - h.length;  // Note that we always have one extra column for the history
    if (columnIndex < offset) return '';
    return h[columnIndex - offset];
  };
                  ...
                  {getCombinedPeopleEntries().map((personEntry) => {
                    ...
                    <tr key={personEntry.id} className={`hover:bg-gray-50 ${
                      ...
                      {Array.from({ length: historyColumnsCount }, (_, index) => {
                        ...
                        const offset = historyColumnsCount - (person!.history?.length ?? 0);
