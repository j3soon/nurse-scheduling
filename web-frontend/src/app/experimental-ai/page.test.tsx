...
vi.mock('@/hooks/useSchedulingData', () => ({
  useSchedulingData: () => ({
    ...
    effectiveExportData: { formatting: [{ type: 'row', people: ['ALL'], backgroundColor: '#fefce8' }] },
...
describe('ExperimentalAiPage', () => {
  ...
  it('sends a question with the current schedule and renders streamed text', async () => {
    ...
    expect(mockGenerateYaml).toHaveBeenCalledWith(expect.objectContaining({
      export: { formatting: [{ type: 'row', people: ['ALL'], backgroundColor: '#fefce8' }] },
    }));
  ...
  it('renders an assistant turn when background optimization wakes the agent', async () => {
    ...
    await waitFor(() => {
      const stored = JSON.parse(window.sessionStorage.getItem('nurse-scheduling-ai-conversation') ?? '{}');
      expect(stored.messages).toEqual(expect.arrayContaining([
        expect.objectContaining({ role: 'optimizer', optimizerJob: { jobId: 'opt-result-1', downloadable: true } }),
      ]));
    });
    firstRender.unmount();
